...
public class PostDao {
    ...
    public List<Post> searchPostsByContent(String content, int offset, int limit) {

        Query<Post> query = MongoUtil.getInstanceDataStore().createQuery(Post.class);
        query.or(
                query.criteria("title").contains(content),
                query.criteria("content").contains(content)
        );
        return query.offset(offset).limit(limit).asList();
    }

    public List<Post> findPostByOption(int offset, int limit, int option) {

        // 0 热门降序
        switch (option) {
            case 0: {
                Query query = MongoUtil.getInstanceDataStore().createQuery(Post.class)
                        .order("-hot")
                        .offset(offset)
                        .limit(limit);
